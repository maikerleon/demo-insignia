...
export const load = async ({ cookies, params }) => {
    ...
    project[0].subactions = subactions && subactions.length > 0 ? true : false;

    if( userData.data.role === 'dependence' ) {
        
        let validateProject = true;
        if( project[0].dependence?.data?.value !== userData.data.dependence ) validateProject = false;

        const hasMatchingActionDependence = project[0].actions.list.some(action => action.dependence?.data?.value === userData.data.dependence);

        if ( !validateProject && !hasMatchingActionDependence ) throw redirect(301, '/proyectos');
...
export const actions = {
    project: async ({request, cookies}) => {
        ...
        try {
            ...
            await registerLogs(userData.data.id, ip, device, `Ha editado las acciones nivel 1 del proyecto ${uniqueID}`);
            await registerLogs(userData.data.id, ip, device, `Ha editado el proyecto ${uniqueID}`);
