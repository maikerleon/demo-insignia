...
export const load = async ({ cookies, params }) => {
    ...
    let { data: project } = await supabase
        .from('projects')
        .select('name, municipalities, type, id')
    ...
    if( userData.data.role === 'dependence' ) {
        ...
        if( action[0].dependence.data.value !== userData.data.dependence ) validateAction = false;

        if ( !validateAction ) throw redirect(301, '/proyectos');
