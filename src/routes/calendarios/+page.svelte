<script>
	...
	const downloadCalendar = () => {
        ...
        const section = document.querySelector('#calendar-section .fc-view-harness.fc-view-harness-passive');
        const scale = 3;
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => {
                ...
                .then((dataUrl) => {
					const content = `
						<section style="padding:20px" id="content-pdf">
							<div style="display: flex;width: 100%;justify-content: space-between;align-items: center;">
								<img src="/logo.webp" alt="logo" style="max-width: 14rem; height:60px;">
								<h1 style="font-weight: 500; color: rgb(56, 56, 56); font-size: 30px;">${type === 'projects' ? 'Proyectos' : 'Acciones'}</h1>
							</div>
					...
					}).from(element).save(`Calendario de ${type === 'projects' ? 'Proyectos' : 'Acciones'} - ${dictionary[currentMonth]} ${currentYear}.pdf`);
                })
                .catch((error) => {
	...
	const downloadCalendarData = async () => {
		...
		let data = type === 'projects' ? [...projects] : [...actions];

		if ( !data || data.length <= 0 ) {
			messageAlert('info', `No existen ${type === 'projects' ? 'proyectos' : 'acciones'} para este mes`);
			return;
		}
		...
		if ( type === 'projects' ) {
			for (const d in data) {
				let element = data[d];
				if( element.progress_supervisor?.data.length > 0 ){
					element.percent_progress = element.progress_supervisor?.data[0].percent;
				}else if ( element.progress_supervisor?.data.length <= 0 && element.progress?.data.length > 0 ) {
					element.percent_progress = element.progress?.data[0].percent;
				}else {
					element.percent_progress = await requestActions(element.id);
				}
			}

		}else {
			data = Object.groupBy(data, ({ project }) => project);
			data = Object.values(data);
		}

		content = data.map(item => generateHTML(item)).join('<div style="page-break-before: always;"></div>');
		...
		}).from(element).save(`${type === 'projects' ? 'Proyectos' : 'Acciones'}.pdf`);
	...
	const generateHTML = (item) => {
		if ( type === 'projects') {
			const municipalities = getMunicipalitiesList(item.municipalities.data);
			return `
				<section style="background-color: white;">
					<div style="padding: 2rem;">
						...
						<div style="width: 100%;border-width: 2px;border-color: black;margin-top: 2.25rem;display: grid;grid-template-columns: repeat(12, minmax(0, 1fr));">
							
							<div style="border-color: black;grid-column: span 12 / span 12;border-bottom-width: 2px;background-color: #22543d;padding: 0.5rem;justify-content: center;">
								<h2 style="text-transform: uppercase;color: white;font-size: 1rem; text-align:center">${item.name}</h2>
							</div>
					
							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;grid-column: span 3 / span 3;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;border-right-width: 2px;justify-content: start;">
								<span style="font-weight: 600;text-transform: uppercase;font-size: 0.9rem;text-align: center;">% de avance</span>
							</div>
							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;grid-column: span 9 / span 9;">
								<span style="font-size: 0.8rem;">${item.percent_progress}%</span>
							</div>

							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;grid-column: span 3 / span 3;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;border-right-width: 2px;justify-content: start;">
								<span style="font-weight: 600;text-transform: uppercase;font-size: 0.95rem;text-align: center;">Inicio</span>
							</div>
							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;grid-column: span 9 / span 9;">
								<span style="font-size: 0.8rem;">${item.start}</span>
							</div>

							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;grid-column: span 3 / span 3;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;border-right-width: 2px;justify-content: start;">
								<span style="font-weight: 600;text-transform: uppercase;font-size: 0.95rem;text-align: center;">Fin</span>
							</div>
							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;grid-column: span 9 / span 9;">
								<span style="font-size: 0.8rem;">${item.end}</span>
							</div>

							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;grid-column: span 3 / span 3;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;border-right-width: 2px;justify-content: start;">
								<span style="font-weight: 600;text-transform: uppercase;font-size: 0.95rem;text-align: center;">Municipios</span>
							</div>
							<div style="display: flex;align-items: center;border-color: black;border-bottom-width: 2px;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;grid-column: span 9 / span 9;">
								<span style="font-size: 0.8rem;">${municipalities}</span>
							</div>
					
							<div style="display: flex;align-items: center;border-color: black;grid-column: span 12 / span 12;border-bottom-width: 2px;background-color: #22543d;padding: 0.5rem;justify-content: center;">
								<h2 style="text-transform: uppercase;color: white;font-size: 0.95rem;">Descripción del proyecto</h2>
							</div>
							<div style="display: flex;align-items: center;grid-column: span 12 / span 12;padding-left: 0.5rem;padding-right: 0.4rem;padding-top: 0.25rem;padding-bottom: 0.25rem;">
								<span style="font-size: 0.8rem; white-space: pre-line;">${item.description ? item.description : 'Sin descripción'}</span>
							</div>
					...
					<div style="padding: 0.5rem;">
						<div style="background-image: url('/figures.webp');background-repeat: repeat;background-size: 150px 150px;width: 100%;height: 5rem;background-color: #2f855a;"></div>
					</div>
				</section>
			`;
		} else {
				...
				<section style="background-color: white;">
						<div style="padding: 2rem;">
							<div style="display: flex;width: 100%;justify-content: space-between;align-items: center; margin-bottom:0.5rem;row-gap:20px">
								<h1 style="text-transform: uppercase;color: black;font-size: 1rem; text-align:left;font-weight: 600;">${item[0].project_name}</h1>
								<img src="/logo.webp" alt="logo" style="max-width: 12rem; height:40px;">
							</div>
			`;
			for (const i in item) {	
	...
	const requestActions = async (id) => {
		try {

			const action = '/api/request-progress-percent-project/'
	...
	const searchCalendar = async () => {
		try {
			...
			handleLoading(`Buscando ${type === 'projects' ? 'proyectos' : 'acciones'}...`);

			const action = '/api/search-calendar/'
	...
	$: options = {
		...
		eventClick: function(info) {
			const urlRedirect = type === 'projects' ? '/proyectos/detalle/' : '/proyectos/accion/';
	...
	const filteredProjects = () => {
		dataSee = type === 'projects' ? [...projects] : [...actions];
...
<div class="sm:p-0 p-3">
	<header class="flex justify-between mb-3 items-center sm:flex-row flex-col w-full">
		<section class="grid gap-2 sm:gap-0 sm:flex items-center justify-start w-full sm:grid-cols-1 grid-cols-3">
			...
			<div class="w-full sm:w-32 max-w-full sm:mr-2">
				<label for="select-project" class="text-xs font-semibold text-gray-600 pl-0.5">Tipo de calendario</label>
				<select class="w-full px-3 pb-2.5 pt-3 text-sm text-gray-600 bg-white border rounded-lg shadow-sm outline-none focus:ring-offset-2 focus:ring-ins-600 focus:ring-2" bind:value={type} on:change={filteredProjects} id="select-project">
					<option value="projects" selected>Proyectos</option>
			...
			<!-- <div class="w-full sm:w-32 max-w-full sm:mr-2">
				...
				<select class="w-full px-3 pb-2.5 pt-3 text-sm text-gray-600 bg-white border rounded-lg shadow-sm outline-none focus:ring-offset-2 focus:ring-ins-600 focus:ring-2" bind:value={priority} on:change={filteredProjects} id="select-priority">
			...
			{#if type === 'actions' }
				<div class="w-full sm:w-36 max-w-full sm:mr-2">
		...
		<section class="pr-2 sm:mt-0 mt-5">
			<h2 class="text-2xl font-semibold text-ins-700 uppercase">{type === 'projects' ? "Proyectos" : "Acciones"}</h2>
		</section>
	</header>

	<div class="mb-4 col-span-3 sm:col-span-1 relative {!search ? 'sm:w-1/5 sm:hover:w-2/5' : 'sm:w-2/5' } transition-all duration-300 w-full">
		...
		<input type="text" bind:value={search} on:keyup={filteredProjects} placeholder="Buscador..." class="text-sm w-full py-2.5 pl-12 pr-4 text-gray-500 border rounded-md outline-none bg-gray-50 focus:bg-white focus:border-ins-700"/>
	</div>
	
	<section class="p-5 rounded-lg bg-white h-fit" id="calendar-section">
		<div id="tooltip" class="custom_tooltip"></div>
