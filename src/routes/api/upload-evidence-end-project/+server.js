...
export async function POST({ request, cookies }) {
	...
	for (const [key, file] of fileEntries) {
		...
		const pathname = `${project}/${crypto.randomUUID()}${extension}`;
	
		const mimeType = mime.lookup(extension) || file.type;

		await supabase.storage
			.from('end_project_evidence')
			.upload(pathname, file, {
				contentType: mimeType
			});

        const publicUrl = await supabase.storage.from('end_project_evidence').getPublicUrl(pathname);
